...
const updateProductInCart = catchAsync(async (req, res) => {
  ...
  if(req.body.quantity>0) 
  {
    ...
    await cartService.deleteProductFromCart(req.user, req.body.productId, req.body.quantity);
 ...
 */
 const checkout = catchAsync(async (req, res) => {
  let con=await cartService.checkout(req.user);
  
  res.status(204).send(con);
});
module.exports = {
  getCart,
  addProductToCart,
